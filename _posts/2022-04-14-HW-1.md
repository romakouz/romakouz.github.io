...
The first step to querying with SQL is to create a database containing all the relevant tables. Here, we will be working with these tables:
...
```python
#we will use pandas for dataframe manipulation and sqlite3 for SQL querying
import pandas as pd
import sqlite3
...
Let's compare countries with different climates. We'll choose India, Denmark, and Egypt, and plot the average yearly temperature distribution across these countries from 190 to 2010.
